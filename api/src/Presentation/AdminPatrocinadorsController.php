...
use App\Application\Admin\Patrocinador\CreatePatrocinadorUseCase;
use App\Application\Admin\Patrocinador\UpdatePatrocinadorUseCase;
use App\Application\Admin\Patrocinador\DeletePatrocinadorUseCase;
use App\Domain\Interfaces\PatrocinadorRepositoryInterface;
use Exception;

class AdminPatrocinadorsController
{
    ...
    public function __construct(
        CreatePatrocinadorUseCase $createUseCase,
        UpdatePatrocinadorUseCase $updateUseCase,
        DeletePatrocinadorUseCase $deleteUseCase,
        PatrocinadorRepositoryInterface $repository
    ) {
    ...
    public function handleRequest(string $method, array $pathParts)
    {
        try {
            ...
            if ($actualMethod === 'POST' && !$id) {
                $this->createUseCase->execute($_POST, $_FILES['logo'] ?? []);
                return $this->jsonResponse(['ok' => true]);
            }
            if ($actualMethod === 'PUT' && $id) {
                $this->updateUseCase->execute($id, $_POST, $_FILES['logo'] ?? null);
                return $this->jsonResponse(['ok' => true]);
            }
